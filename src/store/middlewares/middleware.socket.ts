...
import {
  ...
  disconnectChatuser,
  inactiveChatuser,
} from '../actions/actions.chatusers';
  ...
  (api: MiddlewareAPI) => (next: Dispatch<AnyAction>) => (action) => {
    ...
    switch (action.type) {
      case EReduxUserActionTypes.CONNECT_USER:
        ...
        socket.on(
          'current-users',
          (currentusers: { name: string; joined: Date }[]) => {
        ...
        socket.on(
          'user-connected',
          (newUser: { name: string; joined: Date }) => {
            api.getState().user.data.loggedIn &&
              api.dispatch(
                appendChatuser({ name: newUser.name, joined: newUser.joined })
              );
